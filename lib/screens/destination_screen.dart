      ...
      itemBuilder : ( context, index) {
        ...
        return Card(
            ...
            child: ListTile(
                ...
                title: Text(dest['name'] ?? '', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                subtitle: Text(dest['country'] ?? ''),
                trailing: Icon(Icons.arrow_forward_ios, size: 16),
                onTap: () {
                  // Aquí podrías navegar a una pantalla de detalles o mostrar un diálogo
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('Visitando: ${dest['name']}'))
                  );
                },
            ),
        );
